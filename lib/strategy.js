...
Strategy.prototype.userProfile = function(accessToken, done) {
  ...
  self._oauth2.get(self._profileURL, accessToken, function(err, body) {
    ...
    if (self._includeAllEmails) {
      self._oauth2.get(self._profileURL + '/emails' , accessToken, function (err, body, res) {
        if (err) {
        ...
        if (!json.values || !json.values.length) {
          console.log('Empty json response from Gitlab for user email addresses');
          return done(null, profile);
        }

        var primary_email = profile.emails[0].value; // Default email obtained from previous request
        profile.emails = [];
        (json.values).forEach(function(email) {
          profile.emails.push({ value: email.email, primary: email.email === primary_email });
        });
